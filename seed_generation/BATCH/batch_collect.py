...
def save_output(
    completed: dict, lang: str, version: str, provider: str, output_dir: str
) -> str:
    ts = datetime.now().strftime("%Y%m%d_%H%M%S")
    filename = f"raw_{lang}_{version}_{provider}_{ts}.json"
    path = Path(output_dir) / filename
    nested = {
        "data": {lang: {date: [devo] for date, devo in sorted(completed.items())}}
    }
...
def _print_state_header(state: dict, state_path: str) -> None:
    ...
    print(
        _c(
            row(
                "Dates",
                f"{len(dates)} entries  ({dates[0]} → {dates[-1]})"
                if len(dates) >= 2
                else str(len(dates)),
            ),
            _WHITE,
...
def find_latest_state_file() -> Optional[str]:
    files = sorted(
        _SCRIPT_DIR.glob("batch_state_*.json"), key=os.path.getmtime, reverse=True
    )
    if not files:
        return None
    if len(files) == 1:
    ...
    while True:
        raw = input(
            _c(f"  Select batch [1-{len(files)}]  or  Enter = latest: ", _YELLOW, _BOLD)
        ).strip()
        if raw == "":
...
def collect(state_path: str, results_file: Optional[str] = None) -> None:
    ...
    for result in raw_results:
        ...
        if not result.succeeded:
            ...
            error_records.append(
                {"date": date_key, "reason": "provider_error", "detail": result.error}
            )
            continue
        ...
        if data is None:
            _print_result_row(
                date_key, "PARSE_ERROR", "all JSON repair strategies failed"
            )
            error_records.append(
                {
                    "date": date_key,
        ...
        if not reflexion or not oracion:
            _print_result_row(
                date_key,
                "EMPTY",
                f"reflexion={bool(reflexion)} oracion={bool(oracion)}",
            )
            error_records.append(
                {
                    "date": date_key,
        ...
        if not p1_passed:
            val_warnings.append(
                {"date": date_key, "issue": "phase1", "detail": p1_issues}
            )
        ...
        try:
            devo = build_devotional(
                date_key, seed_entry, master_lang, master_version, reflexion, oracion
            )
            ...
            if p1_issues:
                _print_result_row(
                    date_key,
                    "WARN",
                    f"r:{len(reflexion)} o:{len(oracion)}  {len(p1_issues)} issue(s)",
                )
            else:
                _print_result_row(
    ...
    if completed:
        out_path = save_output(
            completed, master_lang, master_version, provider, output_dir
        )
        print(
            _c(_box_row(f"  {_c('raw output', _CAMO, _BOLD)}   →  {out_path}"), _WHITE)
        )

    if error_records:
        err_path = (
            Path(output_dir)
            / f"batch_errors_{master_lang}_{master_version}_{provider}_{ts}.json"
        )
        _save_json(error_records, err_path)
        print(
            _c(_box_row(f"  {_c('errors    ', _RED, _BOLD)}   →  {err_path}"), _WHITE)
        )

    if val_warnings:
        warn_path = (
            Path(output_dir)
            / f"batch_val_warnings_{master_lang}_{master_version}_{provider}_{ts}.json"
        )
        _save_json(val_warnings, warn_path)
        print(
            _c(
                _box_row(f"  {_c('warnings  ', _ORANGE, _BOLD)}   →  {warn_path}"),
                _WHITE,
            )
        )
    ...
    if error_records:
        print(
            _c(
                f"  !! {len(error_records)} error(s) — run batch_repair.py next:",
                _RED,
                _BOLD,
            )
        )
