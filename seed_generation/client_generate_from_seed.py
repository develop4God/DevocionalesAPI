...
def generate_from_seed(seed_path, master_lang, master_version, output_dir, start_date=None, limit=None):
    SEP = "=" * 60
    print("\n" + SEP)
    ...
    if start_date:
        ...
        total = len(all_dates)
        print("INFO: Partial mode — " + str(len(all_dates)) + " dates | "
    ...
    for i in range(start_index, total):
        ...
        try:
            ...
            print("  " + msg)
            error_dates.append({"date": date_key, "cita": cita, "reason": msg, "completed_before_crash": success_count, "timestamp": datetime.now().isoformat()})
            break
        except Exception as e:
...
def main():
    ...
    root.destroy()
    generate_from_seed(seed_path, master_lang, master_version, output_dir, start_date, limit)
