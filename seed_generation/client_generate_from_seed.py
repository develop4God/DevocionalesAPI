...
def generate_from_seed(
    seed_path, master_lang, master_version, output_dir, start_date=None, limit=None
):
    SEP = "=" * 60
    print("\n" + SEP)
    ...
    # When start_date is given, checkpoint is ignored and only the requested
    # slice of the seed is processed. limit=None means "all from start_date".
    if start_date:
        ...
        try:
            ...
            if resp.status_code == 422:
                ...
                if "SCRIPT_ERROR" in detail:
                    ...
                    error_count += 1
                    error_dates.append(
                        {"date": date_key, "cita": cita, "reason": detail}
                    )
            ...
            if resp.status_code == 503:
                ...
                if not resolved:
                    ...
                    print(
                        f"  503 exhausted — saving checkpoint at [{date_key}] ({cita})"
                    )
                    save_checkpoint(
                        completed,
                        success_count,
            ...
            error_dates.append(
                {
                    ...
                    "cita": cita,
                    "reason": msg,
                    "completed_before_crash": success_count,
                    "timestamp": datetime.now().isoformat(),
                }
            )
            break
...
def main():
    ...
    # --- Fast resume: skip all dialogs if checkpoint exists ---
    checkpoint = load_checkpoint()
    if checkpoint and "output_dir" in checkpoint:
        ...
        if ans == "y":
            ...
            return
        # if "n" — fall through to normal Tkinter dialogs
    ...
    # Read seed now so we can pre-populate start_date dialog with first key
    with open(seed_path, encoding="utf-8") as f:
    ...
    master_lang = simpledialog.askstring(
        "Language", "Language code (e.g. hi, es):", initialvalue=dlang
    )
    if not master_lang:
        sys.exit(0)

    master_version = simpledialog.askstring(
        "Version", "Version code (e.g. OV, HERV):", initialvalue=dver
    )
    ...
    # Leave start_date blank to run full seed / resume from checkpoint.
    # Set limit to a number (e.g. 5) to stop early — useful for test runs.
    ...
    root.destroy()
    generate_from_seed(
        seed_path, master_lang, master_version, output_dir, start_date, limit
    )
