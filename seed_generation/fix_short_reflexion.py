...
with open("2026/seeds/seed_tl_ASND_for_2026.json", encoding="utf-8") as f:
    seed = json.load(f)
with open(
    "2026/yearly_devotionals/Devocional_year_2026_tl_ASND.json", encoding="utf-8"
) as f:
    merged = json.load(f)
...
def build_id(date_key, seed_entry, version):
    ...
    return (
        re.sub(r"\s+", "", cita).replace(":", "") + version + date_key.replace("-", "")
    )
    ...
    for attempt in range(3):
        try:
            ...
            if len(reflexion) < 800:
                print(
                    f"  attempt {attempt + 1}: reflexion too short ({len(reflexion)})",
                    flush=True,
                )
                continue
            devo = {
            ...
            print(
                f"  OK reflexion={len(reflexion)} ends={repr(oracion[-50:])}",
                flush=True,
            )
            saved = True
            break
...
with open(
    "2026/yearly_devotionals/Devocional_year_2026_tl_ASND.json", "w", encoding="utf-8"
) as f:
