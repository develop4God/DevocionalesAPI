"""
API_Server_Seed_Claude.py
─────────────────────────
Seed-driven devotional generation server using Claude Haiku — WITH content validator integration.

New vs Gemini version:
  - Uses Anthropic Claude API (claude-3-5-haiku-20241022)
  - Integrates seed_content_validator.validate_and_fix() after generation:
      ...
      * Script validation: same as Gemini version
  - On Phase 1 failure after auto-fix → retries full Claude generation once more
...
try:
    _CLAUDE_API_KEY = os.environ["ANTHROPIC_API_KEY"]
except KeyError:
...
# =============================================================================
GENERATION_MODEL = (
    "claude-haiku-4-5-20251001"  # Latest Claude Haiku model as of Oct 2025
)

# How many full Claude regeneration attempts before giving up
MAX_CONTENT_RETRIES = 3

# =============================================================================
# SCRIPT VALIDATOR
# Language-aware: checks that Claude responded in the correct script.
# Latin-script languages (de, es, en, fr, pt, tl) are always valid — no check needed.
# =============================================================================
SCRIPT_RANGES = {
...
async def _call_claude_raw(
    ...
    """Raw Claude generation — returns reflexion + oracion. No validation."""
    ...
    print(f"DEBUG: Claude call — verse: {verse_cita}, lang: {lang}")
    ...
    if not reflexion or not oracion:
        raise ValueError(
            f"Claude returned empty reflexion or oracion for '{verse_cita}'"
        )

    return CreativeContent(reflexion=reflexion, oracion=oracion)


async def _call_claude_fix_script(
    verse_cita: str,
    lang: str,
    bad_reflexion: str,
    bad_oracion: str,
...
async def _generate_validated(
    ...
    """
    Full pipeline:
      1. Generate with Claude (up to MAX_CONTENT_RETRIES attempts)
    ...
    for attempt in range(1, MAX_CONTENT_RETRIES + 1):
        ...
        content = await _call_claude_raw(verse_cita, lang, topic)
        ...
        if not r_valid or not o_valid:
            ...
            try:
                content = await _call_claude_fix_script(
...
# =============================================================================
app = FastAPI(
    title="Devotional Seed Generator API (Claude)",
    description="Seed-driven devotional server with Claude Haiku + content validator integration.",
...
async def generate_creative(request: SeedGenerateRequest):
    ...
    Calls Claude Haiku for reflexion + oracion, validates + auto-fixes content,
    ...
    try:
        ...
        raise HTTPException(
            ...
            detail=f"Claude retry exhausted: {str(last)}",
        )
    except Exception as e:
        traceback.print_exc()
        raise HTTPException(
...
if __name__ == "__main__":
    ...
    print("Starting Claude-powered Devotional API Server...")
