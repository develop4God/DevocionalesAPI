...
def generate_devotionals_iteratively():
    ...
    print(
        f"INFO: Iniciando generación de {GENERATION_QUANTITY} devocionales, desde {START_DATE.isoformat()}."
    )
    print("-" * 50)
    ...
    for i in range(GENERATION_QUANTITY):
        ...
        print(
            f"Procesando día {i + 1}/{GENERATION_QUANTITY}: {current_date.isoformat()}..."
        )
        ...
        if VERSIONS_ES_TO_GENERATE:
            filtered_es_versions = [
                v for v in VERSIONS_ES_TO_GENERATE if v != VERSIONS_ES_TO_GENERATE[0]
            ]
        ...
        request_payload = {
            ...
            "master_version": VERSIONS_ES_TO_GENERATE[0]
            if VERSIONS_ES_TO_GENERATE
            else "RVR1960",
        ...
        try:
            ...
            if (
                ...
                and isinstance(json_response["data"], dict)
                and lang_code in json_response["data"]
                and isinstance(json_response["data"][lang_code], dict)
                and date_key in json_response["data"][lang_code]
                and isinstance(json_response["data"][lang_code][date_key], list)
                and len(json_response["data"][lang_code][date_key]) > 0
            ):
                devotional_data = json_response["data"][lang_code][date_key][0]
                # Verificar si el devocional es un error antes de agregarlo
                if (
                    devotional_data.get("id")
                    and not devotional_data.get("id", "").startswith("error_")
                    and "ERROR EN LA GENERACIÓN"
                    not in devotional_data.get("versiculo", "")
                ):
                    successful_devotionals.append(devotional_data)
                    success_count += 1
                    print(
                        f"  -> ÉXITO: Devocional para {current_date.isoformat()} generado y agregado."
                    )
                else:
                    error_count += 1
                    error_message = devotional_data.get(
                        "reflexion", "Error desconocido en la generación."
                    )
                    devotional_id = devotional_data.get("id", "N/A")
                    print(
                        f"  -> ERROR: Devocional '{devotional_id}' para {current_date.isoformat()} falló. Mensaje: {error_message}"
                    )
            else:
                error_count += 1
                print(
                    f"  -> ADVERTENCIA: La respuesta de la API para {current_date.isoformat()} no contiene los datos esperados en el formato correcto."
                )
        except requests.exceptions.Timeout as timeout_err:
            error_count += 1
            print(
                f"  -> ERROR DE TIEMPO DE ESPERA para {current_date.isoformat()}: La API tardó demasiado en responder. {timeout_err}"
            )
        except requests.exceptions.RequestException as req_err:
            error_count += 1
            print(
                f"  -> ERROR en la solicitud para {current_date.isoformat()}: {req_err}"
            )
        except json.JSONDecodeError as json_err:
            error_count += 1
            print(
                f"  -> ERROR de JSON para {current_date.isoformat()}. No se pudo decodificar la respuesta de la API: {json_err}"
            )
        except Exception as e:
            error_count += 1
            print(
                f"  -> ERROR INESPERADO para {current_date.isoformat()}: Tipo: {type(e).__name__}, Mensaje: {e}. Respuesta recibida: {json_response if 'json_response' in locals() else 'N/A'}"
            )
    ...
    print(
        f"Resumen: {success_count} devocionales generados con éxito, {error_count} fallidos."
    )
    ...
    if successful_devotionals:
        ...
        nested_output_data = {}
        lang_code_for_output = (
            LANGUAGES_TO_GENERATE[0] if LANGUAGES_TO_GENERATE else "es"
        )
        ...
        for devocional in successful_devotionals:
            devocional_date = devocional.get(
                "date"
            )  # Obtener la fecha del propio devocional
            if devocional_date:
                ...
                nested_output_data[lang_code_for_output][devocional_date].append(
                    devocional
                )
            else:
                print(
                    f"ADVERTENCIA: Devocional sin clave 'date', no se pudo anidar correctamente: {devocional.get('id', 'N/A')}"
                )
