...
def get_abbreviated_verse_citation(full_verse_citation: str) -> str:
    ...
    match = re.match(
        r"(\d?\s*[A-ZÁÉÍÓÚÜÑa-záéíóüñ]+\s?\d*)\s*(.*)", full_verse_citation
    )
    if not match:
...
def extract_verse_from_content(content: str) -> Optional[str]:
    ...
    match = re.search(
        r"((?:[123]\s)?(?:[A-ZÁÉÍÓÚÜÑa-záéíóüñ]+\s?)+)\s*(\d+:\d+(?:-\d+)?)",
        content,
        re.IGNORECASE,
    )
...
async def generate_devotionals(request: GenerateRequest):
    ...
    while current_date <= request.end_date:
        ...
        try:
            ...
            if (
                main_verse not in excluded_verses
            ):  # Evitar añadir duplicados si ya estaba ahí por algún otro motivo
            ...
            if request.master_lang == "es":
                if date_str not in response_data.es:
                    response_data.es[date_str] = []
                response_data.es[date_str].append(
                    create_error_devocional(
                        current_date,
                        request.master_lang,
                        request.master_version,
                ...
                if date_str not in response_data.en:
                    response_data.en[date_str] = []
                response_data.en[date_str].append(
                    create_error_devocional(
                        current_date,
                        request.master_lang,
                        request.master_version,
