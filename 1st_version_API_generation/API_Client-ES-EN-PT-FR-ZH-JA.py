...
def generate_devotionals_iteratively():
    ...
    print(
        f"INFO: Iniciando generación de {GENERATION_QUANTITY} devocionales en {MASTER_LANG}-{MASTER_VERSION}, desde {START_DATE.isoformat()}."
    )
    print("-" * 50)
    ...
    for i in range(GENERATION_QUANTITY):
        ...
        try:
            ...
            print(
                f"DEBUG: Respuesta recibida: {json.dumps(json_response, indent=2, ensure_ascii=False)}"
            )
            ...
            if (
                ...
                and MASTER_LANG in json_response["data"]
                and isinstance(json_response["data"][MASTER_LANG], dict)
                and date_key in json_response["data"][MASTER_LANG]
                and isinstance(json_response["data"][MASTER_LANG][date_key], list)
                and len(json_response["data"][MASTER_LANG][date_key]) > 0
            ):
                ...
                if (
                    ...
                    success_count += 1
                    print(
                        f"  -> EXITO: Devocional para {current_date.isoformat()} generado y agregado."
                    )
                ...
                if isinstance(json_response, dict) and "data" in json_response:
                    print(
                        f"    Idiomas disponibles en respuesta: {list(json_response['data'].keys()) if isinstance(json_response['data'], dict) else 'N/A'}"
                    )
            ...
            error_count += 1
            print(
                f"  -> ERROR INESPERADO para {current_date.isoformat()}: Tipo: {type(e).__name__}, Mensaje: {e}"
            )
...
if __name__ == "__main__":
    ...
    print(
        f"INFO: Configuración activa - Idioma: {MASTER_LANG}, Versión: {MASTER_VERSION}"
    )
