...
try:
    ...
    raise ValueError(
        "La variable de entorno 'GOOGLE_API_KEY' no está configurada. Asegúrate de tener un archivo .env con tu clave."
    )
        ...
        with open(EXCLUDED_VERSES_FILE, "r", encoding="utf-8") as f:
            try:
                ...
                if isinstance(loaded_verses, list):
                    ...
                    print(
                        f"INFO: Versículos excluidos cargados: {len(excluded_verses)} - {excluded_verses}"
                    )
                else:
                    print(
                        f"ADVERTENCIA: El archivo '{EXCLUDED_VERSES_FILE}' no contiene una lista. Reiniciando lista de excluidos."
                    )
                    excluded_verses = set()
            except json.JSONDecodeError:
                print(
                    f"ERROR: Fallo al decodificar JSON de '{EXCLUDED_VERSES_FILE}'. El archivo puede estar corrupto. Reiniciando lista de excluidos."
                )
                excluded_verses = set()
    else:
        print(
            "INFO: No se encontró el archivo de versículos excluidos. Iniciando con una lista vacía."
        )
        excluded_verses = set()
...
def save_excluded_verses():
    ...
    print(
        f"DEBUG: Versículos excluidos guardados: {len(excluded_verses)} - {excluded_verses}"
    )
...
class DevotionalContent(BaseModel):
    ...
    para_meditar: List[
        ParaMeditarItem
    ]  # <-- CAMBIO AQUÍ: Ahora es una lista de ParaMeditarItem
    oracion: str
    tags: List[str]
...
class LanguageData(BaseModel):
    ...
    zh: Dict[str, List[DevotionalContent]] = Field(
...
def create_error_devocional(
    date_obj: date, lang: str, version: str, error_msg: str
) -> DevotionalContent:
...
def get_abbreviated_verse_citation(full_verse_citation: str) -> str:
    ...
    match = re.match(
        r"(\d?\s*[A-ZÁÉÍÓÚÜÑ a-záéíóüñ]+\s?\d*)\s*(.*)", full_verse_citation
    )
    if not match:
    ...
    for full_name, abbrev in BOOK_ABBREVIATIONS.items():
        if (
            full_name.lower() == book_name_part.lower()
        ):  # Coincidencia exacta (ignorando caso)
            return f"{abbrev} {rest_of_citation}"

        # Si el libro_name_part es como "Corintios" y full_name es "1 Corintios"
        if (
            full_name.lower().endswith(book_name_part.lower())
            and full_name.lower().replace("1 ", "").replace("2 ", "").replace("3 ", "")
            == book_name_part.lower()
        ):
            return f"{abbrev} {rest_of_citation}"
...
def extract_verse_from_content(content: str) -> Optional[str]:
    ...
    if not match:
        print(
            f"DEBUG: No se pudo extraer el patrón de versículo del contenido: {content[:100]}..."
        )
        return None
    ...
    for canonical_name in canonical_nt_books:
        ...
        canonical_name_without_num = (
            canonical_name.replace("1 ", "").replace("2 ", "").replace("3 ", "")
        )
    ...
    if not normalized_book_name:
        ...
        print(
            f"ADVERTENCIA: No se pudo normalizar el nombre del libro '{book_raw}'. Usando tal cual."
        )
    ...
    full_verse = re.sub(
        r"\s*:\s*", ":", full_verse
    )  # Quita espacios alrededor de los dos puntos
    full_verse = re.sub(
        r"\s+", " ", full_verse
    ).strip()  # Normaliza múltiples espacios a uno solo

    print(f"DEBUG: Versículo extraído y normalizado: '{full_verse}'")
    return full_verse


def seleccionar_versiculo_para_generacion(
    excluded_verses_set: set[str], main_verse_hint: Optional[str] = None
) -> str:
    """
    ...
    if not available_verses:
        raise ValueError(
            "No hay versículos disponibles para seleccionar que no estén ya excluidos. Considera limpiar tu lista de excluidos o añadir más versículos posibles."
        )

    # Si hay una pista de versículo y no está excluida, usarla
    if (
        main_verse_hint
        and main_verse_hint in all_possible_verses
        and main_verse_hint not in excluded_verses_set
    ):
        print(f"INFO: Usando versículo principal sugerido (hint): {main_verse_hint}")
        return main_verse_hint
    elif main_verse_hint:
        print(
            f"INFO: El versículo principal sugerido '{main_verse_hint}' está excluido o no es válido/no disponible. Seleccionando uno aleatorio."
        )
...
def assign_devotional_to_language(
    response_data: LanguageData, lang: str, date_str: str, devotional: DevotionalContent
):
...
def get_language_devotionals_list(
    response_data: LanguageData, lang: str, date_str: str
) -> List[DevotionalContent]:
...
@retry(
    ...
    retry=retry_if_exception_type(
        HTTPException
    ),  # Reintentar solo si es una HTTPException (u otros errores de red/API)
)
async def generate_devocional_content_gemini(
    ...
    try:
        ...
        prompt_parts = [
            ...
            "El campo 'versiculo' debe seguir EXACTAMENTE este formato sin excepciones:",
        ...
        if (
            "-" in verse and ":" in verse
        ):  # Detectar si es un rango (ej: "Juan 3:16-18")
            prompt_parts.append("")
            prompt_parts.append(
                "⚠️ ADVERTENCIA IMPORTANTE: Este versículo contiene un RANGO."
            )
            prompt_parts.append(
                "Debes incluir el texto COMPLETO de todos los versículos desde el inicio hasta el final del rango."
            )
            prompt_parts.append(
                f"El campo 'versiculo' DEBE comenzar con: \"{verse} {version}:\""
            )
            prompt_parts.append("NO uses solo el último número del rango.")

        print(
            f"DEBUG: Enviando prompt a Gemini para versículo (abreviado en prompt): {abbreviated_verse_for_prompt} (Original: {verse}) y fecha: {current_date.strftime('%Y-%m-%d')}"
        )
        ...
        extracted_verse_from_response = extract_verse_from_content(
            devocional_data.get("versiculo", "")
        )

        # Comparación más robusta
        if (
            extracted_verse_from_response
            and extracted_verse_from_response.lower() != verse.lower()
        ):
            print(
                f"ADVERTENCIA: El versículo extraído de la respuesta de Gemini ('{extracted_verse_from_response}') no coincide con el versículo solicitado ('{verse}')."
            )
        ...
        if "versiculo" in devocional_data:
            ...
            versiculo_raw = re.sub(
                rf"^\d+\s+(?:JA-)?{re.escape(version)}\s*:\s*", "", versiculo_raw
            )

            # Patrón 3: Quitar duplicación de versión
            versiculo_raw = re.sub(
                rf"{re.escape(version)}\s+{re.escape(version)}", version, versiculo_raw
            )
        ...
        print(
            f"ERROR: Fallo al decodificar JSON de la respuesta de Gemini: {e}. Respuesta: {response.text[:500] if 'response' in locals() else 'No hay respuesta.'}"
        )
        raise HTTPException(
...
async def generate_devotionals(request: GenerateRequest):
    ...
    while current_date <= request.end_date:
        ...
        try:
            main_verse = seleccionar_versiculo_para_generacion(
                excluded_verses, request.main_verse_hint
            )
            master_devocional = await generate_devocional_content_gemini(
                current_date,
                request.master_lang,
                request.master_version,
                main_verse,
                request.topic,
            )
            ...
            assign_devotional_to_language(
                response_data, request.master_lang, date_str, master_devocional
            )
            for (
                lang_to_generate,
                versions_to_generate,
            ) in request.other_versions.items():
                current_lang_date_list = get_language_devotionals_list(
                    response_data, lang_to_generate, date_str
                )
                for version_to_generate in versions_to_generate:
                    if not (
                        lang_to_generate == request.master_lang
                        and version_to_generate == request.master_version
                    ):
                        try:
                            other_version_devotional = (
                                await generate_devocional_content_gemini(
                                    current_date,
                                    lang_to_generate,
                                    version_to_generate,
                                    main_verse,
                            ...
                            print(
                                f"INFO: Devocional generado para {lang_to_generate}-{version_to_generate} con versículo: {main_verse}"
                            )
                        except Exception as e:
                            print(
                                f"ERROR: Fallo al generar devocional para {lang_to_generate}-{version_to_generate} con versículo '{main_verse}': {e}"
                            )
                            current_lang_date_list.append(
                                create_error_devocional(
                                    current_date,
                                    lang_to_generate,
                                    version_to_generate,
                                    f"Fallo en generación de versión adicional: {str(e)}",
                                )
                            )
            ...
            assign_devotional_to_language(
                ...
                date_str,
                create_error_devocional(
                    current_date, request.master_lang, request.master_version, error_msg
                ),
            )
            for (
                lang_to_generate,
                    ...
                    if not (
                        ...
                        current_lang_date_list.append(
                            create_error_devocional(
                                ...
                                lang_to_generate,
                                version_to_generate,
                                f"No generado debido a fallo en selección de versículo maestro: {str(ve)}",
                            )
                        )
        except Exception as e:
            print(
                f"ERROR: Error general al generar la versión maestra para {date_str}: {e}\n{traceback.format_exc()}"
            )
            assign_devotional_to_language(
                response_data,
                request.master_lang,
                ...
                create_error_devocional(
                    ...
                    request.master_lang,
                    request.master_version,
                    f"Error inesperado al generar versión maestra: {str(e)}",
                ),
            )
            for (
                lang_to_generate,
                    ...
                    if not (
                        ...
                        current_lang_date_list.append(
                            create_error_devocional(
                                ...
                                lang_to_generate,
                                version_to_generate,
                                f"No generado debido a error en versión maestra: {str(e)}",
                            )
                        )
